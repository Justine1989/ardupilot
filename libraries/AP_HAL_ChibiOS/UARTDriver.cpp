...
uint8_t UARTDriver::rewrite_read()
{
    return (uint8_t)read();
}
uint16_t UARTDriver::rewrite_available()
{
    return (uint16_t)available();
}
uint16_t UARTDriver::xbee_available()
{
    return data_available();
}
int16_t UARTDriver::xbee_read()
{
    return decode();
}
size_t UARTDriver::xbee_write(const uint8_t chan ,const uint8_t *buffer, size_t size)
{
    uint16_t lenth = 0;
//    if (chan == 2)
//        targ_add = 0xdfdf; //gcs
...
void UARTDriver::xbee_set_targ_add(uint16_t _addr)
{
	targ_add = _addr;
}
//#endif
...
void Xbee::xbee_init(call_read _read, call_available _available,ChibiOS::UARTDriver* _obj)
{
    operating = false;
	success = false;
    datalenth = 0;
